...
type AppendEntriesArgs struct {
	Term int //领导人的任期号
	LeaderId int //领导人的id
	//bk解释：// 预计要从哪里追加的index，因此每次要比当前的len(logs)多1 args初始化为：rf.nextIndex[i] - 1
	PrevLogIndex int //本地Log记录的长度， 初始化为：rf.nextIndex[i] - 1
	//bk解释：追加新的日志的任期号(这边传的应该都是当前leader的任期号 args初始化为：rf.currentTerm
	PrevLogTerm int //本地最后一条Log条目的任期号。用于匹配的日志任期是否合适，是否有冲突
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.votedFor == -1 {
		...
		//论文中第二个投票需要满足要求：1.候选人的日志至少和自己一样新。优先选任期号大的，
		//2.候选人最后一条Log条目的任期号等于本地最后一条Log条目的任期号，则说明同时有两个竞争者，两个一样的网络分区，
		//那么这个时候再看日志条目数，此时候选人args的日志下标就不能低于当前节点的日志下标。
...
func (rf *Raft) persist() {
	// Your code here (2C).
	// Exaple:
	// w := new(bytes.Buffer) 创建一个新的bytes.Buffer实例，bytes.Buffer是一个实现读写方法的可变大小缓冲区，用于保存数据
	// e := labgob.NewEncoder(w) 创建一个新的labgob编码器，将数据编码到之前的缓冲区中，labgob是一个序列化库，用于将复杂的数据结构转换为字节序列，以便存储或网络传输。
	// e.Encode(rf.xxx)
	// e.Encode(rf.yyy)
	// data := w.Bytes()
